...
bool FSRDFeatureDx12::InitNrc()
{
    ...
    nrcBufA = CreateNrcBuffer(Device, UINT64(queryCount) * sizeof(NrcQuery), L"FSRD_NRC_PredIn");
    nrcBufB = CreateNrcBuffer(Device, UINT64(queryCount) * sizeof(float) * 3, L"FSRD_NRC_PredOut");
    nrcBufC = CreateNrcBuffer(Device, UINT64(queryCount) * sizeof(NrcQuery), L"FSRD_NRC_TrainIn");
    nrcBufD = CreateNrcBuffer(Device, UINT64(queryCount) * sizeof(float) * 3, L"FSRD_NRC_TrainTgt");
    nrcBufE = CreateNrcBuffer(Device, sizeof(uint32_t) * 2, L"FSRD_NRC_Counters");

    if (!nrcBufA || !nrcBufB || !nrcBufC || !nrcBufD || !nrcBufE)
    {
    ...
    nrcPredInRes = ffxApiGetResourceDX12(nrcBufA.Get(), FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
    nrcPredOutRes = ffxApiGetResourceDX12(nrcBufB.Get(), FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
    nrcTrainInRes = ffxApiGetResourceDX12(nrcBufC.Get(), FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
    nrcTrainTgtRes = ffxApiGetResourceDX12(nrcBufD.Get(), FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
    nrcCountersRes = ffxApiGetResourceDX12(nrcBufE.Get(), FFX_API_RESOURCE_STATE_UNORDERED_ACCESS);
...
{
    if (_pNrcCtx)
        FfxApiProxy::D3D12_DestroyContext(&_pNrcCtx, NULL);
    _pNrcCtx = nullptr;
    _nrcReady = false;
}

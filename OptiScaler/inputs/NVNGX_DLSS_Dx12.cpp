                                             ...
                                             NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)
{
    ...
    else
    {
        ...
        LOG_INFO("Creating FSR-RR (Ray Regeneration) feature");
    }
    ...
    NVSDK_NGX_FeatureRequirement* OutSupported)
{
    ...
    if (!IdentifyGpu::getPrimaryGpu().dlssCapable && FeatureDiscoveryInfo->FeatureID == NVSDK_NGX_Feature_RayReconstruction)
    {
        ...
        /* ffxQuery can't be used for RR here to check support because this runs before the D3D12Device* is
        captured, and the newer FFX APIs require it to validate support. InitNGXParameters() executes later,
        after the device is available, so full validation can be done there. All this does is allow the game
        to actually check the params instead of failing early. */
        if (FfxApiProxy::IsSRReady() && FfxApiProxy::IsDenoiserReady())
        {
            isRayRegenSupported = true;
    ...
    if (isUpscaling || isRayRegenSupported || (isFG && dlssgAdjacent))
    {
